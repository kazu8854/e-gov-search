...
import { sendStep, sendConclusion, sendDone } from "../shared/ws-notify.mjs";

export const handler = async (event) => {
  const { connectionId, query, articleResults } = event;
  ...
  if (relevantLaws.length === 0) {
    ...
    await sendConclusion(connectionId, {
      summary: "関連する法令条文を特定できませんでした。別のキーワードでお試しください。",
      keyPoints: [],
      relevantLaws: [],
    });

    await sendDone(connectionId);
...
- 法的助言ではなく情報提供であることを明記してください`,
  ...
  // 結論を送信
  await sendConclusion(connectionId, conclusion);
  await sendDone(connectionId);
